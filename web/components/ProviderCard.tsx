...
const STATUS_TEXT: Record<ProviderStatus, string> = {
    operational: "text-[var(--signal-ok)]",
    degraded: "text-[var(--signal-warn)]",
    down: "text-[var(--signal-down)]",
};

const STATUS_GLOW: Record<ProviderStatus, string> = {
    operational: "card-glow-ok",
    degraded: "card-glow-warn",
    down: "card-glow-down",
};
...
export function ProviderCard({
    ...
    /** Zero-based position; drives the stagger delay for the entrance animation. */
    index?: number;
}) {
    const uptime =
        p.uptime_24h !== undefined
    ...
    const pillCls =
        STATUS_PILL[p.current_status] ??
        "bg-[var(--canvas-overlay)] text-[var(--ink-400)]";
    const dotCls = STATUS_DOT[p.current_status] ?? "bg-[var(--ink-500)]";
    const textCls = STATUS_TEXT[p.current_status] ?? "text-[var(--ink-500)]";
    const glowCls = STATUS_GLOW[p.current_status] ?? "card-glow-ok";
    const sparkColor = STATUS_COLOR[p.current_status] ?? "var(--viz-1)";
    const pulse = p.current_status !== "operational";
    const stagger =
        index !== undefined
    ...
    return (
        <Link
            href={`/providers/${p.id}`}
            className={`animate-fade-up card-lift ${glowCls} flex flex-col rounded-lg border
                  border-[var(--ink-600)] bg-[var(--canvas-raised)]
        ...
        >
            ...
            <div className="flex items-center justify-between px-4 py-3.5">
                <div className="flex items-center gap-2.5 min-w-0">
                    {pulse ? (
                        <span
                            className={`status-pulse shrink-0 ${textCls}`}
                            aria-hidden="true"
                        />
                    ) : (
                        <span
                    ...
                    <span className="text-base font-semibold text-[var(--ink-100)] truncate">
                        {p.name}
                    </span>
                </div>
                <span
                    className={`shrink-0 rounded px-2 py-0.5 text-xs font-semibold uppercase tracking-wider ${pillCls}`}
                >
                    {p.current_status}
                </span>
            </div>

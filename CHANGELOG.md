...
- Fix web test suite after Next 16.3 started typechecking `__tests__`: add missing `jest-axe` + `@types/jest-axe` deps, add `types/jest-dom.d.ts` so jest-dom matcher types load (jest.setup.js is outside tsconfig scope), type the optimistic-updates tests explicitly; extend jest ts-jest transform to `.js` so `jest.setup.js`'s ESM import parses
- Fix `RealtimeProvider` infinite re-render: `subscribe`/`unsubscribe` were recreated every render, so consumer effects depending on them re-ran (and dispatched) forever — wrap both in `useCallback`; remove unused `RealtimeAction` import
- Rewrite `__tests__/hooks/useProviderStatus.test.tsx` to stub `useSWR` instead of hitting the network (the old test hung jest); CI web job now runs `npm test`
- `scripts/merge-pr.sh` now waits for CI checks and refuses to merge unless every check is SUCCESS (LLMS-074 — it previously merged #219 while its web job was red, breaking main); tune with `CHECK_WAIT` (default 600s) or bypass with `CHECK_FORCE=1`
